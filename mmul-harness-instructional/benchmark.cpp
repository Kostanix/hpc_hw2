    ...
    for (int64_t n : test_sizes) 
    {
        ...
        for (int b : block_sizes)
        {
          printf("block size: %d\n", b);
           ...
           double mflops = (n * n * n * NUM_OF_OPERATION) / time_sec;  // depends on methods: basic, blocked, etc.

           std::cout << " Elapsed time is : " << time_sec << " " << std::endl;
           std::cout << " MFLOP/s : " << mflops/1e6 << " " << std::endl;
